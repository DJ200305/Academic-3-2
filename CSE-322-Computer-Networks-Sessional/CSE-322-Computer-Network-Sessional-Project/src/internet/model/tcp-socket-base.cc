...
TcpSocketBase::EstimateRtt(const TcpHeader& tcpHeader)
{
    ...
    if (!rtt.IsZero())
    {
        ...
        // RFC 6298, clause 2.4
        m_rto = Max(m_rtt->GetRTO(), m_minRto);
...
TcpSocketBase::ReTxTimeout()
{
    ...
    // QRTT uses timeout events as failure-state feedback.
    if (auto qlearning = DynamicCast<TcpQlearning>(m_rtt))
    {
        qlearning->RecordTimeout(m_rto);
    }
